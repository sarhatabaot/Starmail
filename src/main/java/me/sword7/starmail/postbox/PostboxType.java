package me.sword7.starmail.postbox;


import com.cryptomorin.xseries.XMaterial;

import java.util.UUID;

public enum PostboxType {
    DEFAULT(XMaterial.BROWN_STAINED_GLASS_PANE,
            XMaterial.RED_STAINED_GLASS_PANE,
            XMaterial.GRAY_STAINED_GLASS_PANE,
            UUID.fromString("3b28de42-5619-4737-910f-1a233f7c2878"),
            ""
    ),
    SPACE(XMaterial.MAGENTA_STAINED_GLASS_PANE,
            XMaterial.WHITE_STAINED_GLASS_PANE,
            XMaterial.PINK_STAINED_GLASS_PANE,
            UUID.fromString("60519ffe-0b6f-4591-82f8-e3dd4ac2a7a3"),
            ""
    ),
    RED(XMaterial.RED_STAINED_GLASS_PANE,
            UUID.fromString("39da52c0-f6ff-4f86-a1aa-e3084cdf79af"),
            ""
    ),
    GREEN(XMaterial.GREEN_STAINED_GLASS_PANE,
            UUID.fromString("2c7733e5-cea2-4c7b-b74f-ace493592ae3"),
            ""
    ),
    BLUE(XMaterial.BLUE_STAINED_GLASS_PANE,
            UUID.fromString("b41670db-0a74-4ec7-acc2-28dcdba106fb"),
            ""
    ),
    CUSTOM,
    ;

    private Postbox postbox;

    PostboxType() {

    }

    PostboxType(XMaterial xGlass, UUID uuid, String data) {
        this.postbox = new Postbox(this, this.toString(), xGlass, uuid, data);
    }

    PostboxType(XMaterial xGlass, XMaterial hatBase, XMaterial hatHighlight, UUID uuid, String data) {
        this.postbox = new HatBox(this, this.toString(), xGlass, hatBase, hatHighlight, uuid, data);
    }

    public Postbox getPostbox() {
        return postbox;
    }

}
